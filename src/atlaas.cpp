...
namespace atlaas {
    ...
    void atlaas::infuse_transform(points& cloud, const matrix& transformation, bool dump)
    {
        if (cloud.size() < 1)
            return; // pcl writeBinaryCompressed crash with empty cloud
        if (dump) {
    ...
    void atlaas::infuse_rasterize(const points& cloud)
    {
        cell_info_t zeros{}; // value-initialization w/empty initializer
        std::fill(dyninter.begin(), dyninter.end(), zeros); // fill dyninter with 0
        rasterize(cloud,dyninter);
    }
